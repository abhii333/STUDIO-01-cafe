...
def test_order_records_payment_method_and_awards_coins(client):
    ...
    r = client.post('/api/order', headers=bearer(tok), json={
        'items': [{'name': 'Cold Brew Coffee', 'price': 250, 'quantity': 1}],
        'total': 250, 'coins_to_use': 0, 'payment_id': None, 'payment_method': 'Cash at counter',
    })
...
def test_coins_rejected_for_non_redeemable_item(client):
    ...
    r = client.post('/api/order', headers=bearer(tok), json={
        'items': [{'name': 'Roasted Vegetable Risotto', 'price': 750, 'quantity': 1}],
        'total': 750, 'coins_to_use': 10, 'payment_method': 'Cash at counter',
    })
